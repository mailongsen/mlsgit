...
static DDN *  new_dns_info( unsigned int crc ,int hash , char * d_name) 
{
	...
	//c_name[strlen(d_name)]='\0';
	ddn = (DDN *)kmalloc(sizeof(DDN), GFP_ATOMIC );
	if ( !ddn ){
		return NULL;
	}
	...
	if(iph->protocol == IPPROTO_UDP){
		...
		if(one_time_frag != jf_time_frag){
			...
			if(one_time_frag != jf_time_frag){
				...
				printk(KERN_INFO  "total_times:%d,domain_num:%d,vaild_times:%d",
						atomic_read(&t_packet_times),t_domain_numb,atomic_read(&t_avail_times));
		...
		if( dd_guard_flag ){
			...
			if(n->t_frag != jf_time_frag){
				...
				if( n->t_frag != jf_time_frag){
					...
					//进入分析状态
					if( dd_anyla_flag && atomic_read(&n->r_times) > avge_acce_times){
						n->dflag=1;
